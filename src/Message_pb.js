...
export const Message = $root.Message = (() => {
    ...
    Message.verify = function verify(message) {
        ...
        if (message.serviceId != null && message.hasOwnProperty("serviceId")) {
            let error = $root.ServiceId.verify(message.serviceId);
            if (error)
                return "serviceId." + error;
        }
        if (message.serviceTime != null && message.hasOwnProperty("serviceTime")) {
            let error = $root.ServiceTime.verify(message.serviceTime);
            if (error)
...
export const ServiceTime = $root.ServiceTime = (() => {
    ...
    ServiceTime.decode = function decode(reader, length) {
        if (!(reader instanceof $Reader))
            reader = $Reader.create(reader);
        let end = length === undefined ? reader.len : reader.pos + length, message = new $root.ServiceTime();
        while (reader.pos < end) {
            let tag = reader.uint32();
            switch (tag >>> 3) {
            case 1:
    ...
    ServiceTime.verify = function verify(message) {
        ...
        if (message.response != null && message.hasOwnProperty("response")) {
            ...
            for (let i = 0; i < message.response.length; ++i) {
                let error = $root.ServiceTime.Response.verify(message.response[i]);
                if (error)
    ...
    ServiceTime.fromObject = function fromObject(object) {
        if (object instanceof $root.ServiceTime)
            return object;
        let message = new $root.ServiceTime();
        if (object.now != null)
    ...
    ServiceTime.Response = (function() {
        ...
        Response.decode = function decode(reader, length) {
            if (!(reader instanceof $Reader))
                reader = $Reader.create(reader);
            let end = length === undefined ? reader.len : reader.pos + length, message = new $root.ServiceTime.Response();
            while (reader.pos < end) {
                let tag = reader.uint32();
                switch (tag >>> 3) {
                case 1:
        ...
        Response.fromObject = function fromObject(object) {
            if (object instanceof $root.ServiceTime.Response)
                return object;
            let message = new $root.ServiceTime.Response();
            if (object.from != null)
...
export const ServiceId = $root.ServiceId = (() => {
    ...
    ServiceId.decode = function decode(reader, length) {
        if (!(reader instanceof $Reader))
            reader = $Reader.create(reader);
        let end = length === undefined ? reader.len : reader.pos + length, message = new $root.ServiceId();
        while (reader.pos < end) {
            let tag = reader.uint32();
            switch (tag >>> 3) {
            case 1:
    ...
    ServiceId.fromObject = function fromObject(object) {
        if (object instanceof $root.ServiceId)
            return object;
        let message = new $root.ServiceId();
        if (object.guid != null)
